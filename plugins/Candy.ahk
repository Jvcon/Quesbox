...
Label_Candy_Start:
    ...
    Clipboard := Candy_Saved_ClipBoard  ;还原粘贴板
    Candy_Saved_ClipBoard =
    IniRead,Candy_ProFile_Ini,%g_ConfFile%,Candy,%A_Thishotkey%    ;本热键所调取的配置文件
...
Label_Candy_DrawMenu:
    ...
    CandyMenu_IconSize:=SkSub_IniRead(g_ConfFile, "General_Settings", "MenuIconSize",16)
    CandyMenu_IconDir:=SkSub_IniRead(g_ConfFile, "General_Settings", "MenuIconDir")  ;菜单图标位置
                ...
                If instr(Candy_Cmd,"{ex:")    ; ex：多文件的后缀排除
                    ...
                    If instr(Candy_Cmd,"{input:")   ;特别的参数:带有prompt提示文字的input 例：{Input:请输入延迟时间，以ms为单位},支持多个input输入
                        ...
                        If instr(Candy_Cmd,"{file:name:")
                            ...
                            {
                                all_browser:=SkSub_IniRead(g_ConfFile, "General_Settings", "InUse_Browser")
                                DefaultBrowser:=SkSub_EnvTrans(SkSub_IniRead(g_ConfFile, "General_Settings", "Default_Browser"))
                                ...
                                If Browser ;如果取到了浏览器
                                {
                                    ...
                                    Browser_Args:=SkSub_IniRead(g_ConfFile, "WebBrowser's_CommandLIne", browser_namenoext)
                            ...
                            SkSub_EditConfig(inifile,regex="") ;编辑配置文件！
                            {
                                ...
                                TextEditor:=SkSub_EnvTrans(SkSub_IniRead(g_ConfFile, "General_Settings", "Default_TextEditor"))  ;默认文本编辑器
                                TextEditor:=FileExist(TextEditor) ? TextEditor:"notepad.exe"       ;文本编辑器
                                SplitPath,TextEditor,,,,namenoext
                                LineJumpArgs:=SkSub_IniRead(g_ConfFile, "TextEditor's_CommandLine", namenoext)
                        ...
                        cando_复制保存到Txt中:     ;长文本/短文件
                            TxtPath:=
                            IniRead,TxtPath,%g_ConfFile%,Ky_Settings,Txt保存路径
                            if ErrorLevel
                                TxtPath:=
