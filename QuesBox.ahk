...
g_setting:= JSON.load(g_setting)

if g_setting.runatstart{
    ;TODO: Add run at start
}

for key, label in g_setting.hotkey.global
{
    if (label != "Default")
    {
        Hotkey, %label%, %key%, ,On, UseErrorLevel
        If ErrorLevel  ;热键出错
...
for key, label in g_setting.software
{
    %key%=%label%
}
...
If (g_setting.plugins.capslocks){
    ...
    for key, label in g_setting.hotkey.capslocks{
        if (label != "Default"){
            Hotkey, %label%, %key%, ,On, UseErrorLevel
            If ErrorLevel  ;热键出错
...
Return
    ...
    #Include %A_ScriptDir%\plugins\ActiveandRun.ahk
